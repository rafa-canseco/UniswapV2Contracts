...
contract UniswapV2Twap {
        ...
        returns (uint256 price0Cumulative, uint256 price1Cumulative)
    {
        ...
        if (blockTimestampLast != blockTimestamp) {
            ...
            unchecked {
                ...
                uint256 spotPriceToken0 = uint256(FixedPoint.fraction(reserve1,reserve0)._x);
                uint256 spotPriceToken1 = uint256(FixedPoint.fraction(reserve0,reserve1)._x);
                price0Cumulative += spotPriceToken0 * dt;
                price1Cumulative += spotPriceToken1 * dt;
            }
        }
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        require(tokenIn == token0 || tokenIn == token1,"Not token 0 or token1");
        ...
        if (tokenIn == token0) {
            ...
            amountOut = FixedPoint.decode144(FixedPoint.mul(price0Avg , amountIn));
        } else {
            amountOut = FixedPoint.decode144(FixedPoint.mul(price1Avg,amountIn));
        }
    }
}